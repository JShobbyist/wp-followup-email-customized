...
class FUE_Sending_Mailer {
	...
	public function send_queue_item( $queue_item, $force_send = false ) {
		...
		$email_data['customheader']  = apply_filters('fue_email_customheader', $email_data['customheader'], $email, $queue_item);
		...
		$email_data['customheader']  = apply_filters( 'fue_send_email_customheader', $email_data['customheader'], $queue_item );
		...
		// send the email
		do_action( 'fue_before_email_send', $email_data['subject'], $email_data['customheader'], $email_data['customheader'], $email_data['message'], $headers, $queue_item );
		...
		if ( function_exists('mb_convert_encoding') ) {
			...
			$email_data['customheader'] = preg_replace_callback("/(&#[0-9]+;)/", array( $this, 'convert_encoding' ), $email_data['customheader']);
		}
		...
		if ( !$sent ) {
			...
			self::mail( $email_data['email_to'], $email_data['subject'], $email_data['customheader'], $email_data['message'], $headers );
		}

		do_action( 'fue_after_email_sent', $email_data['subject'], $email_data['customheader'], $email_data['message'], $headers, $queue_item );
	...
	public function send_test_email( $email_data, $email ) {
		...
		do_action( 'fue_before_test_email_send', $email_data['subject'], $email_data['customheader'], $email_data['message'] );

		self::mail( $email_data['email_to'], $email_data['subject'], $email_data['customheader'], $email_data['message'], $headers );

		do_action( 'fue_after_test_email_sent', $email_data['subject'], $email_data['customheader'], $email_data['message'] );
	 ...
	 */
	public static function mail($to, $subject, $customheader, $message, $headers = '', $attachments = '') {
		...
		if ( class_exists('wpMandrill') ) {
			...
			wpMandrill::mail(
				$to, $subject, $customheader, $message, $headers, $attachments,
				array(), // tags
				$from_name, // from_name
			...
			if ( $method == 'wp_mail' ) {
				wp_mail($to, $subject, $customheader, $message, $headers, $attachments);
			} else {
				call_user_func( $method, $to, $subject, $customheader, $message, $headers, $attachments );
			}
	...
	public function send_adhoc_email( $queue_item ) {
		...
		$customheader = apply_filters( 'fue_adhoc_email_customheader', $queue_item->meta['customheader'], $queue_item );
		...
		self::mail( $recipient_email, $subject, $customheader, $message, $headers );
	...
	private function get_email_data( $queue_item, $email ) {
		...
		$data = array(
			...
			'customheader'  => $email->customheader,
		...
		if ( $email->type == 'manual' ) {
			...
			$data['customheader']  = $data['meta']['customheader'];
	...
	private function process_variable_replacements( $email_data, $queue_item = null, $email = null ) {
		...
		$email_data['customheader']  = $variables->apply_replacements( $email_data['customheader'] );
