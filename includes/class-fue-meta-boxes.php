...
class FUE_Meta_Boxes {
	...
	public function __construct() {
		...
		add_filter( 'enter_title_here', array( $this, 'form_title_placeholder' ), 100, 3 );
		add_action( 'edit_form_after_title', array( $this, 'add_subject_field'), 0 );
		add_action( 'edit_form_after_title', array( $this, 'add_customheader_field'), 0 );//this is customheader(aaa)
	...
	public function add_subject_field() {
		...
		<div id="subjectdiv">
			<div id="subjectwrap">
				<label for="post_excerpt" class="fue-label"><?php _e('Email subject', 'follow_up_emails'); ?></label>
	...
	/**
	 * Insert the Customheader field after the Title(aaa)
	 */
	public function add_customheader_field() {
		global $post;
		...
		?>
		<div id="customheaderdiv">
			<div id="customheaderwrap">
				<label for="post_excerpt1" class="fue-label"><?php _e('Email customheader', 'follow_up_emails'); ?></label>
				<label class="screen-reader-text" id="customheader-prompt-text" for="customheader"><?php echo apply_filters( 'enter_customheader_here', __( 'Your customheader line' ), $post ); ?></label>
				<input type="text" placeholder="<?php echo apply_filters( 'enter_customheader_here', __( 'Your customheader line' ), $post ); ?>" name="post_excerpt1" size="30" value="<?php echo esc_attr( htmlspecialchars( $post->post_excerpt1 ) ); ?>" id="post_excerpt1" autocomplete="off" tabindex="0" />
			</div>
		</div>
	...
	public function add_dummy_content( $post ) {
		...
		<div id="email_content_dummy">
			<?php _e("<p>Write your email.</p>", 'follow_up_email'); ?>
		</div>
		<br class="clear" />
		<?php
	}
